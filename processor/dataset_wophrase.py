...
class MMREDataset(Dataset):
    def __init__(self, processor, transform, img_path=None, aux_img_path=None, max_seq=40, sample_ratio=1.0, mode="train") -> None:
        ...
        self.num_labels = len(self.re_dict)  # number of relation labels
    ...
    def __getitem__(self, idx):
        ...
        # 假设 self.num_labels 是关系类别总数
        re_label = torch.zeros(self.num_labels)
        for rel in relation:
            idx = self.re_dict[rel]
            re_label[idx] = 1

        self.aux_img_path = None
        # image process
        if self.img_path is not None:
            try:
                imgid_base = os.path.splitext(imgid)[0]
                img_path = os.path.join(self.img_path, imgid_base + ".JPG")
